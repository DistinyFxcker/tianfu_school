...
class Admins extends Permissions
{
    /**
     * 管理员列表
        ...
        if($id > 0) {
            ...
            if($this->request->isPost()) {
                ...
                if(false == $model->allowField(true)->save($post,['id'=>$id])) {
                    ...
                    return $this->success('修改个人信息成功','admin/admins/personal');
                }
            } else {
                //非提交操作
    	...
    	if($id > 0) {
    		...
    		if($this->request->isPost()) {
	            ...
	            if(false == $model->allowField(true)->save($post)) {
                    ...
                    addlog($model->id);//写入日志
	            	return $this->success('添加管理员成功','admin/admins/index');
	            }
    		} else {
    			//非提交操作
    			$info['admin_cate'] = Db::name('admin_cate')->select();
    		...
    		if($id == Session::get('admin')) {
    			...
    			if(password($post['password_old']) == $admin['password']) {
    				if(false == Db::name('admin')->where('id',$id)->update(['password'=>password($post['password'])])) {
                        ...
                        addlog();//写入日志
    					return $this->success('修改成功','admin/mains/index');
    				}
    			} else {
    ...
    public function delete()
    {
    	if($this->request->isAjax()) {
    		...
    		if(false == Db::name('admin')->where('id',$id)->delete()) {
                ...
                addlog($id);//写入日志
    			return $this->success('删除成功','admin/admins/index');
    		}
    	}
    }
        ...
        if($id > 0) {
            ...
            if($this->request->isPost()) {
                ...
                if(false == $model->allowField(true)->save($post,['id'=>$id])) {
                    ...
                    addlog($model->id);//写入日志
                    return $this->success('修改角色信息成功','admin/admins/adminCate');
                }
            } else {
                //非提交操作
            ...
            if($this->request->isPost()) {
                ...
                if(false == $model->allowField(true)->save($post)) {
                    ...
                    addlog($model->id);//写入日志
                    return $this->success('添加角色成功','admin/admins/adminCate');
                }
            } else {
                //非提交操作
                $menus = Db::name('admin_menu')->select();
        ...
        if($this->request->isAjax()) {
            ...
            if($id > 0) {
                ...
                if(false == Db::name('admin_cate')->where('id',$id)->delete()) {
                    ...
                    addlog($id);//写入日志
                    return $this->success('删除成功','admin/admins/adminCate');
                }
            } else {
                return $this->error('id不正确');
            }
