...
class News extends Permissions
{
    public function index()
    {
        ...
        foreach ($info as $key => $value) {
            $info[$key]['edit_admin'] = Db::name('admin')->where('id',$value['edit_admin_id'])->value('nickname');
            $info[$key]['name'] = Db::name('admin')->where('id',$value['edit_admin_id'])->value('name');
        }
        return $this->fetch('',[
            'info'=>$info,
